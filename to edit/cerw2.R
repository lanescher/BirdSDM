...
#Model 8: cerw (covariate model)
#train: eBird- 1:117936, BBA- 117937:144939, BBS- 144940:177616
#test: eBird- 177617:186833, BBA- 186834:193563, BBS- 193564:201836

m8 <- jagam(cerwdet ~ s(elev, k=10) + s(slope, k=10) + s(temp, k=10) + s(ppt, k=10) + s(can, k=10) + s(dev, k=10) + 
              s(forest, k=10) + s(road, k=10) + cerwct_ebd + lists + s(longitude, latitude, bs='ds', k=100), 
...
outm8 <- jags(data = datm8, parameters.to.save = c("beta","b","y2"), inits = initsm8, 
              model.file = "FinalModels/cerwm8.txt", 
...
save(outm8, m8, m8_dev, Em8_dev, Am8_dev, Sm8_dev, 
     file = "FinalResults/cerwm8.RData")
...
###Model 10: cerw (no eBird)
#train: eBird- 1:117936, BBA- 117937:144939, BBS- 144940:177616,
#test: eBird- 177617:186833, BBA- 186834:193563, BBS- 193564:201836,

m10 <- jagam(cerwdet ~ s(elev, k=10) + s(slope, k=10) + s(temp, k=10) + s(ppt, k=10) + s(can, k=10) + s(dev, k=10) + 
               s(forest, k=10) + s(road, k=10) + s(longitude, latitude, bs='ds', k=100), 
             data = covdat, family='binomial', file = 'placeholder.txt')

datm10 <- list(y = c(covdat$cerwtot[117937:144939], covdat$cerwdet[144940:177616]), 
...
outm10 <- jags(data = datm10, parameters.to.save = c("beta","b","y2"), inits = initsm10, 
               model.file = "FinalModels/cerwm10.txt",  
               n.chains = 3, n.thin = 2, n.adapt = 500, n.burnin = 500, n.iter = 2500)

#Full Deviance
m10_yp <- outm10$mean$y2
m10_yt <- c(covdat$cerwdet[177617:186833], covdat$cerwtot[186834:193563], covdat$cerwdet[193564:201836])
m10_yt <- cbind(m10_yt, c(1 - covdat$cerwdet[177617:186833], 5 - covdat$cerwtot[186834:193563], 1 - covdat$cerwdet[193564:201836]))
...
Em10_yt <- c(covdat$cerwdet[177617:186833])
Em10_yt <- cbind(Em10_yt, c(1 - covdat$cerwdet[177617:186833]))
Em10_yp <- 0.0001 + Em10_yp*0.9998
Em10_dev <- -2*sum(log((Em10_yp^Em10_yt[,1])*((1-Em10_yp)^(Em10_yt[,2]))))
#BBA deviance
Am10_yp <- outm10$mean$y2[9218:15947]
Am10_yt <- c(covdat$cerwtot[186834:193563])
Am10_yt <- cbind(Am10_yt, c(5 - covdat$cerwtot[186834:193563])) 
Am10_yp <- Am10_yp/5
Am10_yp <- 0.0001 + Am10_yp*0.9998
Am10_dev <- -2*sum(log((Am10_yp^Am10_yt[,1])*((1-Am10_yp)^(Am10_yt[,2]))))
#BBS deviance
Sm10_yp <- outm10$mean$y2[15948:24220]
Sm10_yt <- c(covdat$cerwdet[193564:201836])
Sm10_yt <- cbind(Sm10_yt, c(1 - covdat$cerwdet[193564:201836]))
...
save(outm10, m10, m10_dev, Em10_dev, Am10_dev, Sm10_dev, 
     file = "FinalResults/cerwm10.RData")

brier10 <- mean((outm10$mean$y2[9218:24220] - covdat$cerwdet[186834:201836])^2)

pred10 <- prediction(as.numeric(outm10$mean$y2[9218:24220]), covdat$cerwdet[186834:201836])
auc10 <- performance(pred10, measure = "auc")
auc10 <- auc10@y.values[[1]]



###Model 11: cerw (stationary shared, no travelling)
#train: eBird stat- 1:14725, BBA- 14726:41728, BBS- 41729:74405
#test: eBird- 74406:83622, BBA- 83623:90352, BBS- 90353:98625

m11 <- jagam(cerwdet ~ s(elev, k=10) + s(slope, k=10) + s(temp, k=10) + s(ppt, k=10) + s(can, k=10) + s(dev, k=10) + 
               s(forest, k=10) + s(road, k=10) + s(longitude, latitude, bs='ds', k=100), 
             data = covdat2, family='binomial', file = 'placeholder.txt')

datm11 <- list(y = c(covdat2$cerwdet[1:14725], covdat2$cerwtot[14726:41728], covdat2$cerwdet[41729:74405]), 
...
outm11 <- jags(data = datm11, parameters.to.save = c("beta","b","y2"), inits = initsm11, 
               model.file = "FinalModels/cerwm11.txt", 
               n.chains = 3, n.thin = 2, n.adapt = 500, n.burnin = 500, n.iter = 2500, parallel=TRUE)

#Full Deviance
m11_yp <- outm11$mean$y2
m11_yt <- c(covdat2$cerwdet[74406:83622], covdat2$cerwtot[83623:90352], covdat2$cerwdet[90353:98625])
m11_yt <- cbind(m11_yt, c(1 - covdat2$cerwdet[74406:83622], 5 - covdat2$cerwtot[83623:90352], 1 - covdat2$cerwdet[90353:98625]))
...
Em11_yt <- c(covdat2$cerwdet[74406:83622])
Em11_yt <- cbind(Em11_yt, c(1 - covdat2$cerwdet[74406:83622]))
Em11_yp <- 0.0001 + Em11_yp*0.9998
Em11_dev <- -2*sum(log((Em11_yp^Em11_yt[,1])*((1-Em11_yp)^(Em11_yt[,2]))))
#BBA deviance
Am11_yp <- outm11$mean$y2[9218:15947]
Am11_yt <- c(covdat2$cerwtot[83623:90352])
Am11_yt <- cbind(Am11_yt, c(5 - covdat2$cerwtot[83623:90352])) 
Am11_yp <- Am11_yp/5
Am11_yp <- 0.0001 + Am11_yp*0.9998
Am11_dev <- -2*sum(log((Am11_yp^Am11_yt[,1])*((1-Am11_yp)^(Am11_yt[,2]))))
#BBS deviance
Sm11_yp <- outm11$mean$y2[15948:24220]
Sm11_yt <- c(covdat2$cerwdet[90353:98625])
Sm11_yt <- cbind(Sm11_yt, c(1 - covdat2$cerwdet[90353:98625]))
...
save(outm11, m11, m11_dev, Em11_dev, Am11_dev, Sm11_dev, 
     file = "FinalResults/cerwm11_2.RData")

brier11 <- mean((outm11$mean$y2[9218:24220] - covdat2$cerwdet[83623:98625])^2)

pred11 <- prediction(as.numeric(outm11$mean$y2[9218:24220]), covdat2$cerwdet[83623:98625])
...
require(rjags)
jm11 <- jags.model("FinalModels/cerwm11.txt", data=datm11, inits=initsm11, n.adapt=500, n.chains=3)
update(jm11, 500)
sam11 <- jags.samples(jm11, c("b","rho"), n.iter=2000, thin=2)
jam11 <- sim2jam(sam11, m11$pregam)

save(m11, jm11, sam11, jam11,
     file = "FinalResults/Plots/cerwm11_jam.RData")

pdf("FinalResults/Plots/cerw11cov.pdf")
plot(jam11, pages=1)
dev.off()
...
#Plot
pdf("FinalResults/Plots/cerw11map.pdf")
