...
m16 <- jagam(wothdet ~ int + s(elev, k=10) + s(slope, k=10) + s(temp, k=10) + s(ppt, k=10) + s(can, k=10) + s(dev, k=10) + 
               s(forest, k=10) + s(road, k=10) + s(longitude, latitude, bs='ds', k=100), 
             data = dat1, family='binomial', file = 'placeholder.txt')

datm16 <- list(y = c(dat1$wothdet[1:19829], dat1$wothtot[19830:46832], dat1$wothdet[46833:79509]), 
...
outm16 <- jags(data = datm16, parameters.to.save = c("beta","b","y2"), inits = initsm16, 
               model.file = "FinalModels/wothm16.txt", 
               n.chains = 3, n.thin = 2, n.adapt = 500, n.burnin = 500, n.iter = 2500)

#Full Deviance
m16_yp <- outm16$mean$y2
m16_yt <- c(dat1$wothdet[79510:88726], dat1$wothtot[88727:95456], dat1$wothdet[95457:103729])
m16_yt <- cbind(m16_yt, c(1 - dat1$wothdet[79510:88726], 5 - dat1$wothtot[88727:95456], 1 - dat1$wothdet[95457:103729]))
...
Em16_yt <- c(dat1$wothdet[79510:88726])
Em16_yt <- cbind(Em16_yt, c(1 - dat1$wothdet[79510:88726]))
Em16_yp <- 0.0001 + Em16_yp*0.9998
Em16_dev <- -2*sum(log((Em16_yp^Em16_yt[,1])*((1-Em16_yp)^(Em16_yt[,2]))))
#BBA deviance
Am16_yp <- outm16$mean$y2[9218:15947]
Am16_yt <- c(dat1$wothtot[88727:95456])
Am16_yt <- cbind(Am16_yt, c(5 - dat1$wothtot[88727:95456]))
Am16_yp <- Am16_yp/5
Am16_yp <- 0.0001 + Am16_yp*0.9998
Am16_dev <- -2*sum(log((Am16_yp^Am16_yt[,1])*((1-Am16_yp)^(Am16_yt[,2]))))
#BBS deviance
Sm16_yp <- outm16$mean$y2[15948:24220]
Sm16_yt <- c(dat1$wothdet[95457:103729])
Sm16_yt <- cbind(Sm16_yt, c(1 - dat1$wothdet[95457:103729]))
...
save(outm16, m16, m16_dev, Em16_dev, Am16_dev, Sm16_dev, 
     file = "FinalResults/wothm16.RData")

brier16 <- mean((outm16$mean$y2[9218:24220] - dat1$wothdet[88727:103729])^2)

pred16 <- prediction(as.numeric(outm16$mean$y2[9218:24220]), dat1$wothdet[88727:103729])
