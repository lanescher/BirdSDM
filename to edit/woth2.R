...
#Model 8: woth (covariate model)
#train: eBird- 1:117936, BBA- 117937:144939, BBS- 144940:177616
#test: eBird- 177617:186833, BBA- 186834:193563, BBS- 193564:201836

m8 <- jagam(wothdet ~ s(elev, k=10) + s(slope, k=10) + s(temp, k=10) + s(ppt, k=10) + s(can, k=10) + s(dev, k=10) + 
              s(forest, k=10) + s(road, k=10) + wothct_ebd + lists + s(longitude, latitude, bs='ds', k=100), 
            data = covdat, family='binomial', file = 'placeholder.txt')

datm8 <- list(y = c(covdat$wothtot[117937:144939], covdat$wothdet[144940:177616]), 
...
outm8 <- jags(data = datm8, parameters.to.save = c("beta","b","y2"), inits = initsm8, 
              model.file = "FinalModels/wothm8.txt", 
              n.chains = 3, n.thin = 2, n.adapt = 500, n.burnin = 500, n.iter = 2500)

#Full Deviance
m8_yp <- outm8$mean$y2
m8_yt <- c(covdat$wothdet[177617:186833], covdat$wothtot[186834:193563], covdat$wothdet[193564:201836])
m8_yt <- cbind(m8_yt, c(1 - covdat$wothdet[177617:186833], 5 - covdat$wothtot[186834:193563], 1 - covdat$wothdet[193564:201836]))
...
Em8_yt <- c(covdat$wothdet[177617:186833])
Em8_yt <- cbind(Em8_yt, c(1 - covdat$wothdet[177617:186833]))
Em8_yp <- 0.0001 + Em8_yp*0.9998
Em8_dev <- -2*sum(log((Em8_yp^Em8_yt[,1])*((1-Em8_yp)^(Em8_yt[,2]))))
#BBA deviance
Am8_yp <- outm8$mean$y2[9218:15947]
Am8_yt <- c(covdat$wothtot[186834:193563])
Am8_yt <- cbind(Am8_yt, c(5 - covdat$wothtot[186834:193563])) 
Am8_yp <- Am8_yp/5
Am8_yp <- 0.0001 + Am8_yp*0.9998
Am8_dev <- -2*sum(log((Am8_yp^Am8_yt[,1])*((1-Am8_yp)^(Am8_yt[,2]))))
#BBS deviance
Sm8_yp <- outm8$mean$y2[15948:24220]
Sm8_yt <- c(covdat$wothdet[193564:201836])
Sm8_yt <- cbind(Sm8_yt, c(1 - covdat$wothdet[193564:201836]))
...
save(outm8, m8, m8_dev, Em8_dev, Am8_dev, Sm8_dev, 
     file = "FinalResults/wothm8.RData")

brier8 <- mean((outm8$mean$y2[9218:24220] - covdat$wothdet[186834:201836])^2)

pred8 <- prediction(as.numeric(outm8$mean$y2[9218:24220]), covdat$wothdet[186834:201836])
...
###Model 10: woth (no eBird)
#train: eBird- 1:117936, BBA- 117937:144939, BBS- 144940:177616,
#test: eBird- 177617:186833, BBA- 186834:193563, BBS- 193564:201836,

m10 <- jagam(wothdet ~ s(elev, k=10) + s(slope, k=10) + s(temp, k=10) + s(ppt, k=10) + s(can, k=10) + s(dev, k=10) + 
...
outm10 <- jags(data = datm10, parameters.to.save = c("beta","b","y2"), inits = initsm10, 
               model.file = "FinalModels/wothm10.txt",  
               n.chains = 3, n.thin = 2, n.adapt = 500, n.burnin = 500, n.iter = 2500)
...
save(outm10, m10, m10_dev, Em10_dev, Am10_dev, Sm10_dev, 
     file = "FinalResults/wothm10.RData")
...
###Model 11: woth (stationary shared, no travelling)
#train: eBird stat- 1:14725, BBA- 14726:41728, BBS- 41729:74405
#test: eBird- 74406:83622, BBA- 83623:90352, BBS- 90353:98625

m11 <- jagam(wothdet ~ s(elev, k=10) + s(slope, k=10) + s(temp, k=10) + s(ppt, k=10) + s(can, k=10) + s(dev, k=10) + 
               s(forest, k=10) + s(road, k=10) + s(longitude, latitude, bs='ds', k=100), 
...
outm11 <- jags(data = datm11, parameters.to.save = c("beta","b","y2"), inits = initsm11, 
               model.file = "FinalModels/wothm11.txt", 
               n.chains = 3, n.thin = 2, n.adapt = 500, n.burnin = 500, n.iter = 2500)
...
save(outm11, m11, m11_dev, Em11_dev, Am11_dev, Sm11_dev, 
     file = "FinalResults/wothm11.RData")
...
require(rjags)
jm11 <- jags.model("FinalModels/wothm11.txt", data=datm11, inits=initsm11, n.adapt=500, n.chains=3)
update(jm11, 500)
sam11 <- jags.samples(jm11, c("b","rho"), n.iter=2000, thin=2)
jam11 <- sim2jam(sam11, m11$pregam)

save(m11, jm11, sam11, jam11,
     file = "FinalResults/Plots/wothm11_jam.RData")

pdf("FinalResults/Plots/woth11cov.pdf")
plot(jam11, pages=1)
dev.off()
...
#Plot
pdf("FinalResults/Plots/woth11map.pdf")
