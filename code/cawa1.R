...
m5 <- jagam(cawadet ~ s(elev, k=10) + s(slope, k=10) + #s(temp, k=10) + 
              s(ppt, k=10) + s(can, k=10) + s(dev, k=10) + #s(forest, k=10) + 
              s(road, k=10) + s(longitude, latitude, bs='ds', k=100),
...
#Sim2Jam
require(rjags)
# load("results/out/cawam5.RData")

jm5 <- jags.model("models/cawam5.txt",
                  data=datm5,
                  inits=initsm5,
                  n.adapt=500,
                  n.chains=3)
update(jm5, 500)
sam5 <- jags.samples(jm5, c("b","rho"), n.iter=2000, thin=2)
jam5 <- sim5jam(sam5, m5$pregam)

save(m5, jm5, sam5, jam5,
     file = "results/jams/cawam5jam.RData")
...
# Model 6: CAWA (no filtering, no spatial balancing, & no effort filter -----
# #train: eBird- 1:117936, BBA- 117937:144939, BBS- 144940:177616
# #test: eBird- 177617:186833, BBA- 186834:193563, BBS- 193564:201836
# 
