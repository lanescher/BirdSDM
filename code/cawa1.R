...
## ---------------------------
## Objective: Canada Warbler models 1-7
##
## 
## Input:
##   
...
m1 <- jagam(cawadet ~ s(elev, k=10) + s(slope, k=10) + s(temp, k=10) + 
            ...
            data = dat1, family='binomial', file = 'models/placeholder.txt')

datm1 <- list(y = c(dat1$cawadet[1:19829], dat1$cawatot[19830:46832], 
                    dat1$cawadet[46833:79509]), 
...
initsm1 <- function(){
  list(b = m1$jags.ini$b, lambda = m1$jags.ini$lambda,  beta = rep(3, 0.001), 
...
outm1 <- jags(data = datm1, 
              ...
              inits = initsm1, 
              model.file = "Models/Q1Models/cawam1.txt", 
              n.chains = 3, 
              n.thin = 2, 
              n.adapt = 500, 
              n.burnin = 500, 
...
#Full Deviance
m1_yp <- outm1$mean$y2
m1_yt <- c(dat1$cawadet[79510:88726], dat1$cawatot[88727:95456], dat1$cawadet[95457:103729])
m1_yt <- cbind(m1_yt, c(1 - dat1$cawadet[79510:88726], 5 - dat1$cawatot[88727:95456], 1 - dat1$cawadet[95457:103729]))
...
Em1_yt <- c(dat1$cawadet[79510:88726])
Em1_yt <- cbind(Em1_yt, c(1 - dat1$cawadet[79510:88726]))
Em1_yp <- 0.0001 + Em1_yp*0.9998
Em1_dev <- -2*sum(log((Em1_yp^Em1_yt[,1])*((1-Em1_yp)^(Em1_yt[,2]))))
#BBA deviance
Am1_yp <- outm1$mean$y2[9218:15947]
Am1_yt <- c(dat1$cawatot[88727:95456])
Am1_yt <- cbind(Am1_yt, c(5 - dat1$cawatot[88727:95456]))
Am1_yp <- Am1_yp/5
Am1_yp <- 0.0001 + Am1_yp*0.9998
Am1_dev <- -2*sum(log((Am1_yp^Am1_yt[,1])*((1-Am1_yp)^(Am1_yt[,2]))))
#BBS deviance
Sm1_yp <- outm1$mean$y2[15948:24220]
Sm1_yt <- c(dat1$cawadet[95457:103729])
Sm1_yt <- cbind(Sm1_yt, c(1 - dat1$cawadet[95457:103729]))
...
#roc1 <- roc(dat1$cawadet[88642:103748], as.numeric(outm1$mean$y2[9218:24220]), ci.auc=TRUE, auc=TRUE, parallel=TRUE)
...
#Sim2Jam
require(rjags)
load("results/cawam1.RData")

jm1 <- jags.model("Models/Q1Models/cawam1.txt", 
                  data=datm1, 
                  inits=initsm1, 
                  n.adapt=500, 
                  n.chains=3)
...
datm2 <- list(y = c(dat2$cawadet[1:20361], dat2$cawatot[20362:47364], 
                    dat2$cawadet[47365:80041]), 
...
outm2 <- jags(data = datm2, 
              parameters.to.save = c("beta","b","y2"), 
              inits = initsm2, 
              model.file = "FinalModels/cawam2.txt", 
...
save(outm2, m2, m2_dev, Em2_dev, Am2_dev, Sm2_dev, 
     file = "FinalResults/cawam2.RData")
...
##Model 3: CAWA (no eBird spatial balancing)
#train: eBird- 1:111728, BBA- 111729:138731, BBS- 138732:171408
#test: eBird- 171409:180625, BBA- 180626:187355, BBS- 187356:195628

m3 <- jagam(cawadet ~ s(elev, k=10) + s(slope, k=10) + s(temp, k=10) + s(ppt, k=10) + s(can, k=10) + s(dev, k=10) + 
...
outm3 <- jags(data = datm3, parameters.to.save = c("beta","b","y2"), inits = initsm3, 
              model.file = "FinalModels/cawam3.txt", 
              n.chains = 3, n.thin = 2, n.adapt = 500, n.burnin = 500, n.iter = 2500)
...
save(outm3, m3, m3_dev, Em3_dev, Am3_dev, Sm3_dev, 
     file = "FinalResults/cawam3.RData")
...
##Model 4: CAWA (no eBird filtering or spatial balancing)
#train: eBird- 1:117936, BBA- 117937:144939, BBS- 144940:177616
#test: eBird- 177617:186833, BBA- 186834:193563, BBS-193564:201836

m4 <- jagam(cawadet ~ s(elev, k=10) + s(slope, k=10) + s(temp, k=10) + s(ppt, k=10) + s(can, k=10) + s(dev, k=10) + 
              s(forest, k=10) + s(road, k=10) + s(longitude, latitude, bs='ds', k=100), 
            data = dat4, family='binomial', file = 'placeholder.txt')

datm4 <- list(y = c(dat4$cawadet[1:117936], dat4$cawatot[117937:144939], dat4$cawadet[144940:177616]), 
