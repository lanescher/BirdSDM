...
#Model 12: cerw (full model, validate on current; train1test2)
#train: eBird- 1:19829, BBA- 19830:46832, BBS- 46833:79509,
#test: eBird- 79510:84969, BBS- 84970:87201

m12 <- jagam(cerwdet ~ s(elev, k=10) + s(slope, k=10) + s(temp, k=10) + s(ppt, k=10) + s(can, k=10) + s(dev, k=10) + 
               s(forest, k=10) + s(road, k=10) + s(longitude, latitude, bs='ds', k=100), 
             data = dat12, family='binomial', file = 'placeholder.txt')

datm12 <- list(y = c(dat12$cerwdet[1:19829], dat12$cerwtot[19830:46832], dat12$cerwdet[46833:79509]), 
...
outm12 <- jags(data = datm12, parameters.to.save = c("beta","b","y2"), inits = initsm12, 
               model.file = "FinalModels/Q2Models/cerwm12.txt", 
               n.chains = 3, n.thin = 2, n.adapt = 500, n.burnin = 500, n.iter = 2500)

#Full Deviance
m12_yp <- outm12$mean$y2
m12_yt <- c(dat12$cerwdet[79510:84969], dat12$cerwdet[84970:87201])
m12_yt <- cbind(m12_yt, c(1 - dat12$cerwdet[79510:84969], 1 - dat12$cerwdet[84970:87201]))
m12_yp <- 0.0001 + m12_yp*0.9998
m12_dev <- -2*sum(log((m12_yp^m12_yt[,1])*((1-m12_yp)^(m12_yt[,2]))))
#eBird deviance
Em12_yp <- outm12$mean$y2[1:5460]
Em12_yt <- c(dat12$cerwdet[79510:84969])
Em12_yt <- cbind(Em12_yt, c(1 - dat12$cerwdet[79510:84969]))
Em12_yp <- 0.0001 + Em12_yp*0.9998
Em12_dev <- -2*sum(log((Em12_yp^Em12_yt[,1])*((1-Em12_yp)^(Em12_yt[,2]))))
#BBS deviance
Sm12_yp <- outm12$mean$y2[5461:7692]
Sm12_yt <- c(dat12$cerwdet[84970:87201])
Sm12_yt <- cbind(Sm12_yt, c(1 - dat12$cerwdet[84970:87201]))
...
save(outm12, m12, m12_dev, Em12_dev, Sm12_dev, 
     file = "FinalResults/cerwm12.RData")

brier12 <- mean((outm12$mean$y2 - dat12$cerwdet[79510:87201])^2)

pred12 <- prediction(as.numeric(outm12$mean$y2), dat12$cerwdet[79510:87201])
auc12 <- performance(pred12, measure = "auc")
auc12 <- auc12@y.values[[1]]



#Model 13: cerw (no older data or BBA; hdat)
#train: eBird- 1:10748, BBS- 10749:19556
#test: eBird- 19557:25016, BBS- 25017:27248

m13 <- jagam(cerwdet ~ s(elev, k=10) + s(slope, k=10) + s(temp, k=10) + s(ppt, k=10) + s(can, k=10) + s(dev, k=10) + 
               s(forest, k=10) + s(road, k=10) + s(longitude, latitude, bs='ds', k=100), 
...
outm13 <- jags(data = datm13, parameters.to.save = c("beta","b","y2"), inits = initsm13, 
               model.file = "FinalModels/Q2Models/cerwm13.txt", 
...
save(outm13, m13, m13_dev, Em13_dev, Sm13_dev, 
     file = "FinalResults/cerwm13.RData")
...
#Model 14: cerw (old data and BBA as covariate; hdat)
#train: eBird- 1:10748, BBS- 10749:19556
#test: eBird- 19557:25016, BBS- 25017:27248

m14 <- jagam(cerwdet ~ s(elev, k=10) + s(slope, k=10) + s(temp, k=10) + s(ppt, k=10) + s(can, k=10) + s(dev, k=10) + 
               s(forest, k=10) + s(road, k=10) + s(longitude, latitude, bs='ds', k=100) +
               lists_ebd + lists_bba + lists_bbs + cerwct_ebd + cerwct_bba + cerwct_bbs, 
             data = hdat, family='binomial', file = 'placeholder.txt')

datm14 <- list(y = c(hdat$cerwdet[1:10748], hdat$cerwdet[10749:19556]), 
...
outm14 <- jags(data = datm14, parameters.to.save = c("beta","b","y2"), inits = initsm14, 
               model.file = "FinalModels/Q2Models/cerwm14.txt", 
               n.chains = 3, n.thin = 2, n.adapt = 500, n.burnin = 500, n.iter = 2500, parallel=TRUE)

#Full Deviance
m14_yp <- outm14$mean$y2
m14_yt <- c(hdat$cerwdet[19557:25016], hdat$cerwdet[25017:27248])
m14_yt <- cbind(m14_yt, c(1 - hdat$cerwdet[19557:25016], 1 - hdat$cerwdet[25017:27248]))
m14_yp <- 0.0001 + m14_yp*0.9998
m14_dev <- -2*sum(log((m14_yp^m14_yt[,1])*((1-m14_yp)^(m14_yt[,2]))))
#eBird deviance
Em14_yp <- outm14$mean$y2[1:5460]
Em14_yt <- c(hdat$cerwdet[19557:25016])
Em14_yt <- cbind(Em14_yt, c(1 - hdat$cerwdet[19557:25016]))
Em14_yp <- 0.0001 + Em14_yp*0.9998
Em14_dev <- -2*sum(log((Em14_yp^Em14_yt[,1])*((1-Em14_yp)^(Em14_yt[,2]))))
#BBS deviance
Sm14_yp <- outm14$mean$y2[5461:7692]
Sm14_yt <- c(hdat$cerwdet[25017:27248])
Sm14_yt <- cbind(Sm14_yt, c(1 - hdat$cerwdet[25017:27248]))
...
save(outm14, m14, m14_dev, Em14_dev, Sm14_dev, 
     file = "FinalResults/cerwm14.RData")

brier14 <- mean((outm14$mean$y2 - hdat$cerwdet[19557:27248])^2)

pred14 <- prediction(as.numeric(outm14$mean$y2), hdat$cerwdet[19557:27248])
auc14 <- performance(pred14, measure = "auc")
auc14 <- auc14@y.values[[1]]


#Model 15: cerw (BBA false pos; dat12)
#train: eBird- 1:19829, BBA- 19830:46832, BBS- 46833:79509
#test: eBird- 79510:84969, BBS- 84970:87201

m15 <- jagam(cerwdet ~ s(elev, k=10) + s(slope, k=10) + s(temp, k=10) + s(ppt, k=10) + s(can, k=10) + s(dev, k=10) + 
               s(forest, k=10) + s(road, k=10) + s(longitude, latitude, bs='ds', k=100), 
             data = dat12, family='binomial', file = 'placeholder.txt')

datm15 <- list(y = c(dat12$cerwdet[1:19829], dat12$cerwtot[19830:46832], dat12$cerwdet[46833:79509]), 
...
outm15 <- jags(data = datm15, parameters.to.save = c("beta","alpha1","alpha3","b","y2","f1"), inits = initsm15, 
               model.file = "FinalModels/Q2Models/cerwm15.txt", 
               n.chains = 3, n.thin = 2, n.adapt = 500, n.burnin = 500, n.iter = 2500)

#Full Deviance
m15_yp <- outm15$mean$y2
m15_yt <- c(dat12$cerwdet[79510:84969], dat12$cerwdet[84970:87201])
m15_yt <- cbind(m15_yt, c(1 - dat12$cerwdet[79510:84969], 1 - dat12$cerwdet[84970:87201]))
m15_yp <- 0.0001 + m15_yp*0.9998
m15_dev <- -2*sum(log((m15_yp^m15_yt[,1])*((1-m15_yp)^(m15_yt[,2]))))
#eBird deviance
Em15_yp <- outm15$mean$y2[1:5460]
Em15_yt <- c(dat12$cerwdet[79510:84969])
Em15_yt <- cbind(Em15_yt, c(1 - dat12$cerwdet[79510:84969]))
Em15_yp <- 0.0001 + Em15_yp*0.9998
Em15_dev <- -2*sum(log((Em15_yp^Em15_yt[,1])*((1-Em15_yp)^(Em15_yt[,2]))))
#BBS deviance
Sm15_yp <- outm15$mean$y2[5461:7692]
Sm15_yt <- c(dat12$cerwdet[84970:87201])
Sm15_yt <- cbind(Sm15_yt, c(1 - dat12$cerwdet[84970:87201]))
...
save(outm15, m15, m15_dev, Em15_dev, Sm15_dev, 
     file = "FinalResults/cerwm15.RData")

brier15 <- mean((outm15$mean$y2 - dat12$cerwdet[79510:87201])^2)

pred15 <- prediction(as.numeric(outm15$mean$y2), dat12$cerwdet[79510:87201])
...
require(rjags)
jm14 <- jags.model("FinalModels/Q2Models/cerwm14.txt", data=datm14, inits=initsm14, n.adapt=500, n.chains=3)
update(jm14, 500)
sam14 <- jags.samples(jm14, c("b","rho"), n.iter=2000, thin=2)
jam14 <- sim2jam(sam14, m14$pregam)

save(m14, jm14, sam14, jam14,
     file = "FinalResults/Plots/cerwm14_jam.RData")

pdf("FinalResults/Plots/cerw14cov.pdf")
plot(jam14, pages=1)
dev.off()

load("S:/MillerLab/Projects/BirdSDM/FinalResults/Plots/grid_pa_1km_v2.RData")
library(ggplot2)
library(sf)
library(sp)
...
gridcerw$pred <- predict(jam14, newdata = gridcerw, type="response")
grid_sp <- as_Spatial(gridcerw)
...
#Plot
pdf("FinalResults/Plots/cerw14map.pdf")
